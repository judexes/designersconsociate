    <!-- HERO SECTION -->
    <section id="top" class="hero-section shade">
        <div class="splash view-height-100 view-height-0-sm center-children center-children-0-sm">
            <div class="container home2">
                <div class="row pt-5 pt-sm-0">
                    <div class="offset-sm-1 col-sm-5 mt-sm-5">
                        <div class="hero-description pb-5 pt-5 text-center">
                            <span class="title pt-5 pt-sm-0  ">We are committed to developing sustainable solutions </span>
                        </div>
                    </div>
                    <div class="col-sm-5 mt-sm-5 ">
                        <div class="hero-description pb-5 pt-5 text-center dark title">
                            <span class="">that invigorate the African Fashion eco system</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>